...
#     def __init__(self, tokenizer, args, N):
...
#         weight = torch.sum(last_hidden_state*attn.unsqueeze(-1),1)/torch.sum(attn,1,keepdim=True)#TODO change to use the first hidden state, one FC layer is enough
...
class attention_params(torch.nn.Module):
    def __init__(self, tokenizer, args, N):
        super(attention_params, self).__init__()
        self.alpha = torch.nn.Parameter(torch.normal(mean=torch.zeros(N),std=0.1))
        self.act =  torch.nn.Sigmoid()
        
    def forward(self, x, attn, idx):
        weight = self.alpha[idx]
        weight = self.act(weight)
        # weight = torch.clamp(weight, min=0.5,max=1.5)
...
#     def __init__(self, tokenizer, args, N):
#         super(attention_params, self).__init__()
#         self.alpha = torch.nn.Parameter(torch.ones(N)/N)#torch.normal(mean=torch.zeros(N),std=0.1))
#         self.act =  torch.nn.Softmax(-1)
        
#     def forward(self, x, attn, idx):
        
#         probs = self.act(self.alpha)
